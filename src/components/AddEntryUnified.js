...
function AddEntry({ onAdd }) {
    ...
    const handleSearch = async (type) => {
        ...
        try {
            ...
            let options = {};
            if (type === 'book') {
                url = `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(input)}&key=${process.env.NEXT_PUBLIC_GOOGLE_BOOKS_API_KEY}`;
            } else if (type === 'movie') {
                url = `https://www.omdbapi.com/?apikey=${process.env.NEXT_PUBLIC_OMDB_API_KEY}&s=${encodeURIComponent(input)}`;
            } else if (type === 'album') {
    ...
    const fetchPageTitle = async (url) => {
        try {
            ...
            const data = await response.json();
            return { title: data.title, favicon: data.favicon, bodyText: data.bodyText };
        } catch (error) {
            console.error('Error fetching page title:', error);
            return { title: url, favicon: null, bodyText: '' };
        }
    };

    const handleAdd = async (item) => {
        // Determine whether the item is a book, movie, album, podcast, link, or plaintext based on its structure
        if (item.volumeInfo) {
            // Book structure
            onAdd({
                type: 'book',
